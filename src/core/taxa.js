...
function init() {
  ...
  for (const row of rows) {
    if (
      ...
      row.nameType === "scientific" &&
      row.taxonRank in includeTaxa
    ) {
      byScientific[row.canonicalName] = row;
    }
  }
  console.log(
    `${Object.keys(byScientific).length} taxa records kept out of ${
      rows.length
    }.`
  );
}
...
function loadTreeSearch() {
  ...
  console.log(
    `${Object.keys(treeSearch).length} rows loaded for Global Tree Search.`
  );
}
