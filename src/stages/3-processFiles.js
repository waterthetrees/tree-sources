...
const cleanTree = require("../core/cleanTree");
const taxa = require("../core/taxa");
...
const optionList = [
  {
    ...
    type: Boolean,
    defaultValue: false,
    description: "Use a combined allout.geojson file",
  },
  { name: "sources", type: String, multiple: true, defaultOption: true },
  {
    ...
    type: String,
    multiple: true,
    description: "Restrict processing to these countries.",
  },
  {
    name: "force",
    alias: "f",
  ...
  .filter(
    (s) =>
      ...
      options.country.find(
        (c) => (s.country || "").toLowerCase() === c.toLowerCase()
      )
  );
...
function processTree(source, tree) {
  ...
  for (const prop of Object.keys(crosswalk)) {
    let val =
      typeof crosswalk[prop] === "function"
        ? crosswalk[prop](origProps)
        : origProps[crosswalk[prop]];
    if (val !== undefined) {
...
function addSpeciesCount(tree) {
  if (tree.genus && tree.species) {
    ...
    sourceTaxoCount[tree.source].species[scientific] =
      1 + (sourceTaxoCount[tree.source].species[scientific] || 0);
  }
}
...
if (options.allout) {
  ...
  console.log(
    `Individually processing temporary files from tmp/out_* into out/*.nd.geojson`
  );
}
...
try {
  ...
  console.warn(
    "No taxoCount.json found. Regenerate it after processing with 3a-updateTaxoCount.js."
      .yellow
  );
}
...
async function loadSource(source, out) {
  ...
  return new Promise((resolve, reject) => {
    ...
    fs.createReadStream(inName)
        ...
        try {
          if (source.coordsFunc) {
            ...
            if (
              ...
              tree.geometry.coordinates.length < 1
            ) {
              tree.geometry = null;
            }
          }
      ...
      .on("end", () => {
        console.log(
          `Processed ${keepCount} from ${source.id.cyan} (dropped ${delCount}, ${noGeomCount} had no geometry)`
        );
        sourceStats[source.id] = {
          ...
          delCount,
          noGeomCount,
          differentSpecies: Object.keys(sourceTaxoCount[source.id].species)
            .length,
          speciesCounts: Object.keys(sourceTaxoCount[source.id].species)
            .filter(
              (species) =>
                sourceTaxoCount[source.id].species[species] > keepCount / 100
            ) // keep any species that is at least X% of the total
            .map((species) => [
              species,
              sourceTaxoCount[source.id].species[species],
            ])
...
async function loadSources() {
  ...
  if (options.allout) {
    outFile = fs
      .createWriteStream("tmp/allout.json")
      ...
      .map((source) => {
        if (options.allout) {
          ...
          if (fs.existsSync(outName) && !options.force) {
            ...
            const localOutfile = fs
              .createWriteStream(outName)
              .on("error", console.error);
...
async function processSources() {
  ...
  const stats = {
    ...
    keptTrees: sources.reduce(
      (total, { id }) => total + (sourceStats[id].keepCount || 0),
      0
    ),
    countries: [
      ...new Set(sources.map((s) => s.country).filter(Boolean)).keys(),
    ],
  };
  ...
  console.log(
    `${stats.openTrees.toLocaleString()} open data trees from ${
      stats.sources
    } sources.`
  );
  ...
  fs.writeFileSync(
    "./sourceTaxoCount.json",
    JSON.stringify(sourceTaxoCount, null, 2)
  );
