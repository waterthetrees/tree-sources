...
async function process() {
  const out = fs
    .createWriteStream("tmp/allout-species.json")
  ...
  fs.createReadStream(`tmp/allout.json`)
    ...
    .on("end", () => {
      console.log(
        `Added species counts in ${perf.stop("process").words}, renaming files.`
      );

      child_process.execSync(
        "mv tmp/allout.json tmp/allout-without-species.json"
      );
