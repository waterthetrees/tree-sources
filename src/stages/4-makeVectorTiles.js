...
const optionList = [
  {
    name: "separate-mbtiles",
    type: Boolean,
    defaultValue: false,
    description: "Generate individual mbtilesfiles.",
  },
  {
    name: "single-source",
    type: Boolean,
  ...
  },
  {
    ...
    type: Boolean,
    defaultValue: false,
    description: "Skip generation, just join existing mbtiles.",
  },
...
if (options["separate-mbtiles"] || options["just-join"]) {
  if (options["separate-mbtiles"]) {
    ...
    for (let file of files) {
      ...
      if (
        !fs.existsSync(mbtilesFile) ||
        fs.existsSync(`${mbtilesFile}-journal`)
      ) {
