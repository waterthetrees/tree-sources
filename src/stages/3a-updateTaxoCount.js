...
function countSpecies({ properties: tree }) {
  if (tree.genus && tree.species) {
    taxoCount.species[tree.genus + " " + tree.species] =
      1 + (taxoCount.species[tree.genus + " " + tree.species] || 0);
  }
  if (tree.family) {
    taxoCount.class[tree.class] = 1 + (taxoCount.class[tree.class] || 0);
    taxoCount.subclass[tree.subclass] =
      1 + (taxoCount.subclass[tree.subclass] || 0);
...
fs.createReadStream(`tmp/allout.json`)
  ...
  .on("end", () => {
    fs.writeFile(
      "taxoCount.json",
      JSON.stringify(taxoCount),
      () => "taxoCount.json written"
    );
